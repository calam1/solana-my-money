...
describe('mymoneydapp', () => {
  ...
  it('Is initialize test state', async () => {
    mint = await createMint(provider, program.authority);
...
// Ideally, while testing this contract, we should have been able to simply use @solana/web3.js to interact with our Anchor program, but it so happens that Anchor 
// is built off a lot of components from Serum and is thus dependent on Serum. What that also means is that, we cannot simply import and use @solana/web3.js until 
// @project-serum/serum uses the same version of @solana/web3.js as anchor.
// So, to mimick the functions from @solana/web3.js, simply copy and paste the following code. Consider this code as the SPL token client boilerplate for test initialization. 
